...
3. **Keyword Search (RAG)** — BM25-inspired TF-IDF keyword search; at query time only the top-5 most relevant docs are injected into the prompt. (Vector embeddings not available via Replit AI proxy.)
4. **Knowledge Base Manager** — CRUD for KB entries, file upload (PDF/TXT/MD/DOCX), type tagging, SharePoint folder browser
5. **File Import** — Upload PDF/TXT/Markdown/DOCX files; text extracted server-side using `pdf-parse`, `mammoth`, multer
6. **SharePoint Integration (FedAuth)** — Browse SharePoint "Anyone with the link" shared folders via FedAuth cookie trick (no Azure credentials needed); all 44 Cam360 KB files imported
